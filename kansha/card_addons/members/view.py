...
def render_member_friend(self, h, comp, *args):
    with h.a.action(comp.answer, self):
        h << self.user.render(h, 'avatar')
    return h.root
...
@presentation.render_for(Member, 'remove')
def render_member_remove(self, h, comp, model):
    return self.user.render(h, 'remove')
