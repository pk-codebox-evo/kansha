...
class DataMember(Entity):
    ...
    role = Field(Unicode(255), default=lambda: u'member')
    ...
    def get_board_members(cls, board):
        return cls.get_board_users_by_role(board, u'member')

    @classmethod
    def is_board_member(cls, board, user):
        return cls.exists(board=board, user=user, role=u'member')

    @classmethod
    def has_board_managers(cls, board):
        return cls.exists(board=board, role=u'manager')
    ...
    @classmethod
    def add_board_user(cls, board, user, role=u'member'):
    ...
    @classmethod
    def change_board_user_role(cls, board, user, role=u'member'):
        member = cls.get_by(board=board, user=user)
        member.role = role
